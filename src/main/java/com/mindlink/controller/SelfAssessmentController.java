...
public class SelfAssessmentController {
    ...
    public SelfAssessmentController(AssessmentService assessmentService, UserService userService) {
    ...
    public String result(@PathVariable String typeKey,
        ...
        if ("burnout".equals(typeKey)) {
            return burnoutResult(assessment, questions, params, maxChoiceScore, user, model);
        }
        ...
        assessmentService.saveResult(user, typeKey, assessment.getName(), totalScore, level, highRisk);
        model.addAttribute("assessment", assessment);
    ...
    private String burnoutResult(AssessmentType assessment, List<AssessmentQuestion> questions,
                                  Map<String, String> params, int maxChoiceScore, User user, Model model) {
        ...
        assessmentService.saveResult(user, assessment.getTypeKey(), assessment.getName(),
                Math.max(personalScore, workScore),
                "개인 " + personalLevel + " · 업무 " + workLevel, highRisk);
        model.addAttribute("assessment", assessment);
