...
- **한 파일**: [sql/ORACLE_SETUP.sql](sql/ORACLE_SETUP.sql) — 스키마 보정, `users`(role 포함)·`book_reviews`, 김동주 `data.sql`과 동일한 **관리자·공지·커뮤니티 시드**, `recommendation_books` 시드까지 순서대로 포함.
- **실행**: `docs/LoginCommunity.md`의 DDL로 테이블이 준비된 뒤, SQL Developer에서 `APP_USER`로 전체 실행. (추천도서만 갈아엎을 때는 파일 안내에 따라 섹션 3만 선택 실행 가능.)
- **H2 로컬**: `src/main/resources/data.sql`은 게시글/댓글만 두고, `spring.sql.init.mode`를 프로필에 맞게 조정.
...
**Oracle 스키마**: `recommendation_books.search_keyword` 등은 [sql/ORACLE_SETUP.sql](sql/ORACLE_SETUP.sql) **1번 구간**에서 조건부로 추가·보정합니다.
...
- 초기 샘플 공지·관리자·게시글: Oracle은 `sql/ORACLE_SETUP.sql` 섹션 2b, H2는 `data.sql`(프로필에서 init 설정 시)
