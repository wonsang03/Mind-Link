...
--
-- 전제: 앱을 최소 1회 기동해 Hibernate가 기본 테이블을 만든 뒤 실행하는 것을 권장.
--       (테이블이 없으면 아래 ALTER 구문에서 오류가 날 수 있음)
--
-- 구성
--   1) 스키마 보정 — 문자 길이, CLOB, recommendation_books.search_keyword 등
--      (posts 등 커뮤니티 테이블이 없으면 docs/LoginCommunity.md §5 DDL을 먼저 실행)
--   2) users(없을 때만, role 포함) + book_reviews 생성 + 기존 users에 role 컬럼 보정
--   2b) 샘플 데이터 — 관리자 계정·공지 3건·커뮤니티 게시글/댓글 (김동주 data.sql 기반)
...
-- =============================================================================
-- 2. USERS(없을 때만) + book_reviews
-- spring.jpa.hibernate.ddl-auto=none 이면 users 미생성으로 FK 오류가 날 수 있음.
-- =============================================================================
...
-- =============================================================================
-- 2b. 샘플 시드 (김동주 브랜치 data.sql 동일 내용, Oracle 문법)
--     notices / posts / post_comments 테이블이 있어야 합니다. 없으면 docs/LoginCommunity.md DDL 선행.
--     관리자: admin@mindlink.com / 비밀번호 admin1234 (BCrypt)
-- =============================================================================
